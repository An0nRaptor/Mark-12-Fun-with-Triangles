...
function checkButtonHandler() {
  ...
  if ((sides[0].value !== "") && (sides[1].value !== "")) {
    ...
    if ((Math.sign(side1) === 1) && (Math.sign(side2) === 1)) {
      ...
      showOutputText("Length of sides can't be negative. Please enter positive lengths in both fields.");
    }
    
    else if ((Math.sign(side1) === 0) || (Math.sign(side2) === 0)) {
      showOutputText("Length of sides can't be zero. Please enter positive lengths in both fields.");
    }
